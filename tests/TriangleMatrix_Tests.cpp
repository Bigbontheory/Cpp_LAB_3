...
TEST(TriangleMatrixTest, Constructors) {
    TriangleMatrix<int> m1(3, TriangleType::Lower);
    EXPECT_EQ(m1.get_size(), 3);
    EXPECT_EQ(m1.get_type(), TriangleType::Lower);

    int arr[] = { 1, 2, 3, 4, 5, 6 };
    // Нижнетреугольная 3x3: [1,0,0], [2,3,0], [4,5,6]
    TriangleMatrix<int> m2(arr, 3, TriangleType::Lower);
    EXPECT_EQ(m2.get(0, 0), 1);
    EXPECT_EQ(m2.get(1, 1), 3);
    EXPECT_EQ(m2.get(0, 1), 0);

    // Верхнетреугольная 3x3: [1,2,3], [0,4,5], [0,0,6]
    TriangleMatrix<int> m3(arr, 3, TriangleType::Upper);
...
TEST(TriangleMatrixTest, SetMethod) {
    TriangleMatrix<int> m_low(3, TriangleType::Lower);
    m_low.set(2, 0, 10);
    EXPECT_EQ(m_low.get(2, 0), 10);
    EXPECT_THROW(m_low.set(0, 2, 5), std::invalid_argument);

    TriangleMatrix<int> m_up(3, TriangleType::Upper);
    m_up.set(0, 2, 20);
...
TEST(TriangleMatrixTest, MathOperations) {
    ...
    TriangleMatrix<double> m(arr, 2, TriangleType::Lower);
...
TEST(TriangleMatrixTest, Addition) {
    ...
    TriangleMatrix<int> tri(arr, 2, TriangleType::Lower);
...
TEST(TriangleMatrixTest, BoundsAndExceptions) {
    EXPECT_THROW(TriangleMatrix<int>(-1, TriangleType::Lower), std::invalid_argument);

    TriangleMatrix<int> m(2, TriangleType::Upper);
